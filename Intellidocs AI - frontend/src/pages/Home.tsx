    ...
    <div className="max-w-4xl mx-auto space-y-12 py-12 px-6">
      ...
      {mutation.isError && (
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          className="p-6 bg-rose-50 border border-rose-200 rounded-3xl flex items-center gap-4"
        >
          <AlertTriangle className="w-5 h-5 text-rose-500 flex-shrink-0" />
          <div>
            <p className="text-sm font-bold text-rose-700">Connection Error</p>
            <p className="text-xs text-rose-500 mt-1">
              Make sure the FastAPI backend is running on port 8000. Run: <code className="bg-rose-100 px-1.5 py-0.5 rounded">uvicorn main.main:app --reload</code>
            </p>
          </div>
        </motion.div>
      )}
